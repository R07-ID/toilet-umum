/*
* JANGAN UBAH-UBAH INFO-NYA YA TOD NANTI ERROR NANGIS!!!
* "JANGAN MODAL NAMA DOANG YA TOD!!!"
...
const { ROOTBOT, tanggal, waktu, instagram, whatsapp, youtube, nomer, aktif, ontime } = config
...
const
{
   ...
   waChatKey,
    ...
    const menu1 = require('./menu1')
    const menu2 = require('./menu2')
    const menu3 = require('./menu3')
    const info = require('./info')
    const donate = require('./donate')
    const alay = require('./alay')
    const artinama = require('./artinama')
    const corona = require('./corona')
    const kbbi = require('./kbbi')
    const downloadImage = require('./downloadImage')
    const igStalk = require('./igStalk')
    const jodoh = require('./jodoh')
    const jsholat = require('./jsholat')
    const lirik = require('./lirik')
    const nulis = require('./nulis')
    const radTextInImage = require('./readTextInImage')
    const pantun = require('./pantun')
    ...
    const bucin = require('./bucin')
    const cersex = require('./cersex')
    const cerpen = require('./cerpen')
    const puisi1 = require('./puisi1')
    const puisi2 = require('./puisi2')
    const resep = require('./resep')
    const namaninja = require('./namaninja')
    const bitly = require('./bitly')
    const nekonime = require('./nekonime')
    const cektanggal = require('./cektanggal')
    const chord = require('./chord')
    const zodiak = require('./zodiak')
    const fb = require('./fb')
    const simi = require('./simi')
    const wikien = require('./wikien')
    const spamsms = require('./spamsms')
    const spamcall = require('./spamcall')
    const spamgmail = require('./spamgmail')
    const covidcountry = require('./covidcountry')
    const infoanime = require('./infoanime')
    const gay = require('./gay')
    const ytmp3 = require('./ytmp3')
    const ssweb = require('./ssweb')
    const infogempa = require('./infogempa')
    const indohot = require('./indohot')
...
client.on('message-status-update', json => {
   ...
   console.log(`[ ${time} ] => bot by :@TnDj4R07`)
})
...
client.on('message-new', async (m) => {
   ...
   switch (prefix) {
       case 'help':
           client.sendMessage(id, help.help(id, ROOTBOT, tanggal, waktu, whatsapp, youtube, instagram, aktif, nomer, ontime), MessageType.text)
           break
       case 'menu1':
           client.sendMessage(id, menu1.menu1(id, ROOTBOT, tanggal, waktu, whatsapp, youtube, instagram, aktif, nomer, ontime), MessageType.text)
           break
       case 'menu2':
           client.sendMessage(id, menu2.menu2(id, ROOTBOT, tanggal, waktu, whatsapp, youtube, instagram, aktif, nomer, ontime), MessageType.text)
           break
       case 'menu3':
           client.sendMessage(id, menu3.menu3(id, ROOTBOT, tanggal, waktu, whatsapp, youtube, instagram, aktif, nomer, ontime), MessageType.text)
           break
      case 'donate':
           client.sendMessage(id, donate.donate(id, ROOTBOT, tanggal, waktu, whatsapp, youtube, instagram, aktif, nomer, ontime), MessageType.text)
           break          
      case 'info':
           client.sendMessage(id, info.info(id, ROOTBOT, tanggal, waktu, whatsapp, youtube, instagram,aktif, nomer, ontime), MessageType.text)
           break             
       case 'nulis':
           nulis(value)
               .then(data => {
                   client.sendMessage(id, '#}{❗}{# Tunggu Sebentar🙏!!.....', MessageType.text)
       ...
       case 'igstalk':
           igStalk(value)
               .then(data => {
                   ...
                   client.sendMessage(id, '}{WAIT!!}{ Sebentar Ya Tod...⏳', MessageType.text)
  ...
  case 'indohot':
           indohot()
               .then(data => {
                   ...
                   let hasil = `Loe Mau Tobat Kapan G-B-L-K💆‍♂️* \n\n *Judul* _${judul}_ \n\n *Status* _${genre}_ \n\n *Durasi* _${durasi}_ \n\n *Link Bosq* _${url}_  `
                   client.sendMessage(id, hasil, MessageType.text)
               })
               .catch(err => {
                   console.log(err)
